...
import { BedDouble, Clock, ShieldCheck, ArrowLeft } from "lucide-react"

export const metadata = { title: "Indoor Patient Services & Ward Rules | NINS&H" }

export default function IndoorPatientPage() {
  return (
        ...
        <div className="mx-auto max-w-7xl px-6 space-y-10">
          <div className="flex items-center justify-between border-b border-slate-200/80 pb-4 dark:border-white/10">
            <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
              <Link href="/">Home</Link><span>/</span><span>Services</span><span>/</span><span className="text-foreground font-bold">Indoor Patient</span>
            </div>
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#0f172a] via-[#1e293b] to-[#2563eb] p-8 lg:p-12 text-white shadow-2xl">
            <div className="relative z-10 space-y-3 max-w-3xl">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3.5 py-1 text-xs font-semibold text-blue-200">
                <BedDouble className="h-3.5 w-3.5" /> 500-Bed Inpatient Care
              </div>
              <h1 className="font-heading text-3xl font-extrabold tracking-tight sm:text-4xl lg:text-5xl">Indoor Patient Care &amp; Admission Wards</h1>
              <p className="text-sm text-slate-200">Admission procedures, free bed quota for underprivileged patients, visiting hours (4:00 PM - 6:00 PM), and cabin allocation.</p>
            </div>
          </div>
        </div>
      </main>
