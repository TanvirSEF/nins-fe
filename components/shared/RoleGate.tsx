...
export function RoleGate({ children, allowedRoles }: RoleGateProps) {
  ...
  if (allowedRoles && allowedRoles.length > 0) {
    ...
    if (!hasAccess) {
      return (
        <div className="flex min-h-screen items-center justify-center bg-background p-6">
          <div className="w-full max-w-md border border-destructive/20 bg-card rounded-2xl p-8 shadow-lg transition-all duration-300 hover:shadow-xl dark:border-destructive/10">
            <div className="flex flex-col items-center text-center">
              {/* Emergency indicator circle */}
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-destructive/10 text-destructive mb-6 animate-pulse">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed mb-6">
                Your role (<span className="font-mono text-xs font-semibold px-2 py-0.5 rounded bg-muted text-foreground">{role}</span>) is not authorized to access this section of the NINS Enterprise Portal.
              </p>
